...
async function handleCommand(msg) {
  ...
  switch (method) {
    ...
    case "set_file_input":
      return await cmdDomInMainWorld("set_file_input", params);
  ...
  switch (method) {
    case "evaluate": {
      try {
        ...
        return { __xhs_error: `JS 执行错误：${e.message}` };
      }
    }
  ...
  switch (method) {
    ...
    case "input_content_editable": {
      return new Promise(async (resolve) => {
        const el = document.querySelector(params.selector);
        if (!el) {
          resolve({ __xhs_error: `元素不存在：${params.selector}` });
          return;
        }
        el.focus();

        // 直接设置 innerHTML，比 execCommand 快得多
        el.innerHTML = params.text.replace(/\n/g, "<br>");

        // 触发 input 事件，让页面知道内容变化了
        el.dispatchEvent(new Event("input", { bubbles: true }));
        el.dispatchEvent(new Event("change", { bubbles: true }));
    ...
    case "set_file_input": {
      return new Promise((resolve) => {
        const el = document.querySelector(params.selector);
        if (!el) {
          resolve({ __xhs_error: `文件输入框不存在：${params.selector}` });
          return;
        }
        ...
        // 方法 1: 覆盖 files 属性 + change 事件（标准 file input）
        try {
          const dt = makeFiles();
        ...
        // 方法 2: drag-drop 到上传区域（XHS 主要监听 drop 事件）
        const dropTarget =
